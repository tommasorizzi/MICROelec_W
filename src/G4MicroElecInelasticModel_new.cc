  ...
  :G4VEmModel(nam),isInitialised(false)
{
  ...
  G4cout << ">>> Using MyMicroElecInelasticModel!" << G4endl;
}
...
//corrections CI 07/02/2022 - removed
/*
  ...
  */
  //corrections CI 07/02/2022 - removed
  //corrections CI 07/02/2022 - addeed
	G4bool weaklyBound = currentMaterialStructure->IsShellWeaklyBound(shell);
	...
	if (!weaklyBound) {
		...
		// correction CI 09/03/2022 removing - currentMaterialStructure->GetInitialEnergy()
		//secondaryElectronKineticEnergy = transf - currentMaterialStructure->GetLimitEnergy(shell) - currentMaterialStructure->GetInitialEnergy();
		...
		if (secondaryElectronKineticEnergy <= 0.) {
			//secondaryElectronKineticEnergy = transf - currentMaterialStructure->GetEnergyGap();
			secondaryElectronKineticEnergy = 0.0;
			SEFromFermiLevel = true;
		}
	}

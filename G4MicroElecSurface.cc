    ...
    theGlobalNormal(0.,0.,0.), theFacetNormal(0.,0.,0.)
{
  ...
  G4cout << ">>> Using MyMicroElecSurfaceModel!" << G4endl;
}
...
void G4MicroElecSurface::Initialise()
{
	...
	for (G4int i = 0; i < numOfCouples; ++i) {
		...
		G4cout << this->GetProcessName() << ", Material " << i + 1 << " / " << numOfCouples << " : " << material->GetName() << G4endl;
...
{
	// Correction CI -12/01/2023 - foolowing  code removed replaced by following new one
  /*
  ...
  if (valid)
  ...
  else
    {
	 ...
	 << G4endl;
      G4Exception("G4MicroElecSurf::PostStepDoIt", "OpBoun01",
		  FatalException, ed,
  ...
  return G4VDiscreteProcess::PostStepDoIt(aTrack, aStep);
  */
  // Correction CI -12/01/2023 - previous code removed
  // Correction CI -12/01/2023 -replace previous code 
  if (!isInitialised) {Initialise();}
    ...
    if (pPostStepPoint->GetStepStatus() != fGeomBoundary || pPostStepPoint->GetPhysicalVolume()->GetName()==pPreStepPoint->GetPhysicalVolume()->GetName())
	{
	    theStatus = NotAtBoundarySurf;
		flag_franchissement_surface = false;
		flag_reflexion = false;
	...
	if (aTrack.GetStepLength()<=kCarTolerance/2 * 0.0000000001)
	{
		//G4cout << "������������������������ TELEPORT ���������������������������" << G4endl;
		if (flag_reflexion == true)
		{
			//G4cout << "has been reflected just before" << G4endl;
			flag_reflexion = false;
			return G4VDiscreteProcess::PostStepDoIt(aTrack, aStep);
		...
		if (pPreStepPoint && pPostStepPoint) {
			...
			else
			{
				...
				G4String matNew = pPostStepPoint->GetMaterial()->GetName();
				G4String matOld = pPreStepPoint->GetMaterial()->GetName();
				...
				// if (matNew == "G4_W") {
				// G4cout << "[DEBUG] Overriding post-step work function for G4_W: using 10 eV" << G4endl;
				// G4cout.flush();
				// }
				// if (matOld == "G4_W") {
				// G4cout << "[DEBUG] Overriding pre-step work function for G4_W: using 10 eV" << G4endl;
				// G4cout.flush();
				// }
				// G4cout << "[DEBUG] StepTooSmall: thresholdNew = " << thresholdNew_surface / eV
				// 		<< " eV, thresholdOld = " << thresholdOld_surface / eV
				// 		<< ", energyThreshold = " << (thresholdNew_surface - thresholdOld_surface) / eV << " eV" << G4endl;
				// 		G4cout.flush();
		...
		if (aStep.GetPreStepPoint()->GetMaterial()->GetName() != aStep.GetPostStepPoint()->GetMaterial()->GetName()) {
	...
	flag_normal = (theGlobalNormal == G4ThreeVector(0, 0, 1) || theGlobalNormal == G4ThreeVector(0, 0, -1));
	...
	if ((thePrePV)&&(thePostPV))
	{
		...
		else
		{
			...
			G4String matNew = thePostPV->GetLogicalVolume()->GetMaterial()->GetName();
			G4String matOld = thePrePV->GetLogicalVolume()->GetMaterial()->GetName();

			G4double thresholdNew = (matNew == "G4_W") ? 9.5 * eV : postStepWF->second;
			G4double thresholdOld = (matOld == "G4_W") ? 9.5 * eV : preStepWF->second;

			// if (matNew == "G4_W") {
			// G4cout << "[DEBUG] Overriding post-step threshold for G4_W: using 10 eV" << G4endl;
			// G4cout.flush();
			// }
			// if (matOld == "G4_W") {
			// G4cout << "[DEBUG] Overriding pre-step threshold for G4_W: using 10 eV" << G4endl;
			// G4cout.flush();
			// }
			// G4cout << "[DEBUG] MainThreshold: thresholdNew = " << thresholdNew / eV
			// 	<< " eV, thresholdOld = " << thresholdOld / eV
			// 	<< ", energyThreshold = " << (thresholdNew - thresholdOld) / eV << " eV" << G4endl;
			// 	G4cout.flush();
	...
	thetaft=std::asin(std::sqrt(ekint/(ekint+energyThreshold))*std::sin(thetat));//Angle de r�fraction
	...
	//Parameter for an exponential barrier of potential (Th�se P68)
	G4double at=0.5E-10;
    ...
    if((aleat<=crossingProbability)&&(ekint> energyDelta))
	{
		...
		// G4cout << "[ESCAPED] Electron passed random draw." << G4endl;
		// G4cout << "→ Energy before loss: " << ekint / eV << " eV" << G4endl;
		// G4cout << "→ energyDelta (WF barrier): " << energyDelta / eV << " eV" << G4endl;
		// G4cout << "→ Final KE after escape: " << (ekint - energyDelta) / eV << " eV" << G4endl;
		// G4cout << "→ crossingProbability: " << crossingProbability << G4endl;
		// G4cout << "→ aleat (random draw): " << aleat << G4endl;
		// G4cout << "→ Transmitted through surface!" << G4endl << G4endl;
	...
	{
		// G4cout << "[REFLECTED] Electron failed random draw." << G4endl;
		// G4cout << "→ Energy: " << ekint / eV << " eV" << G4endl;
		// G4cout << "→ energyDelta (WF barrier): " << energyDelta / eV << " eV" << G4endl;
		// G4cout << "→ crossingProbability: " << crossingProbability << G4endl;
		// G4cout << "→ aleat (random draw): " << aleat << G4endl;
		// G4cout << "→ Reflected back into material." << G4endl << G4endl;
		...
		G4cout << "energie trop faible" << G4endl;
